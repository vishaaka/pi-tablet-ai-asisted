...
from config.settings import assets_dir, service_settings, service_url, ui_path
...
from storage.asset_history import list_asset_history
from storage.asset_manager import (
    create_asset,
    list_assets,
    get_asset,
    update_asset,
...
from storage.command_queue import push_command, get_pending_commands
from storage.draft_manager import add_draft, get_all_drafts, get_draft, delete_draft
from storage.mode_manager import get_mode, set_mode
from system.server_manager import (
    start_server,
    stop_server,
    start_all,
    stop_all,
...
PANEL_SERVICE = service_settings("panel")
HTML_PATH = ui_path()
ASSETS_PATH = assets_dir()

# NOT:
# /assets API endpointleri ile çakışmaması için statik mount yolu /asset_files yapıldı
app.mount("/asset_files", StaticFiles(directory=str(ASSETS_PATH)), name="asset_files")
...
def root():
    return {
        "panel": "ok",
        "open": service_url("panel", PANEL_SERVICE.get("open_path", "/panel")),
        "mode": get_mode(),
    }
...
def panel_api_summary():
    return safe_get_json(service_url("dashboard", "/api/summary"))


@app.get("/panel/api/milestones")
def panel_api_milestones():
    return safe_get_json(service_url("dashboard", "/api/milestones"))


@app.get("/panel/api/ai_decision")
def panel_api_ai_decision():
    return safe_get_json(service_url("tool", "/ai_decision"))


if __name__ == "__main__":
    import uvicorn
    uvicorn.run(
        PANEL_SERVICE["uvicorn_app"],
        host=PANEL_SERVICE["host"],
        port=int(PANEL_SERVICE["port"]),
        reload=False,
    )
