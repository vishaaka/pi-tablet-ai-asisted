...
$Ports = @{
    ...
    media = [int]$Settings.services.media.port
}
...
$Commands = @{
    ...
    media      = "python -m uvicorn $($Settings.services.media.uvicorn_app) --host $($Settings.services.media.host) --port $($Settings.services.media.port)"
}
...
function Show-Status {
    foreach ($name in @('panel','tool','middleware','dashboard','media')) {
        $port = $Ports[$name]
...
if ([string]::IsNullOrWhiteSpace($Action)) {
    ...
    Write-Host "  manage_servers.bat start media"
    ...
    Write-Host "  manage_servers.bat stop media"
